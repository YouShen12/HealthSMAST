...
# load dataset
Heart = pd.read_csv('diabetes_mod.csv', sep=',')
target = Heart.pop('target') # num in raw dataset
heart_feature_names = ['age', 'gender', 'polyuria', 'polydipsia','polyphagia','obesity']
X = Heart[heart_feature_names].values
Y = target.values
...
def get_basic_model():
  model = tf.keras.Sequential([
    tf.keras.layers.Dense(6, input_dim=6, activation='relu'),
